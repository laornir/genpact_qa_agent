...
-- Example: courses offered in spring with teacher and room (Postgres ILIKE for case-insensitive match)
SELECT c.name AS course, t.name AS teacher, o.room, sem.name AS semester
FROM course_offerings o
JOIN courses c  ON c.id  = o.course_id
JOIN teachers t ON t.id  = o.teacher_id
JOIN semesters sem ON sem.id = o.semester_id
WHERE sem.season::text ILIKE 'spring'
ORDER BY sem.year, c.name;
